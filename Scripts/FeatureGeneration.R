...
compute_features <- function(window_chunk, features_list) {
  ...
  accel_axes <- intersect(available_axes, c("Accelerometer.X", "Accelerometer.Y", "Accelerometer.Z"))
...
process_data <- function(relabelled_data, 
  ...
  # define each window
  starts <- seq(1, nrow(relabelled_data), by = (window_samples - overlap_samples))
  
  # process the windows using multiprocessing via future_lapply
  ...
  # Reset to sequential for the rest of the processing
  plan(sequential)
