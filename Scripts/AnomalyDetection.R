...
# set up ####
library(pacman)
p_load(tidyverse, data.table, e1071)
...
unsup_1_class_SVM_results <- function(
  ...
  training_data <- process_data(split$training, features_list, window_length = window_length, 
  ...
  validation_data <- process_data(split$validation, features_list, window_length = window_length, 
                                  overlap_percent = overlap_percent, down_Hz = down_Hz, 
                                  feature_normalisation = feature_normalisation)
  ...
  reference_training <- as.factor(ifelse(training_data_labels$Activity == "Locomotion", "Normal", "Outlier"))
  ...
  reference_validation <- as.factor(ifelse(validation_data_labels$Activity == "Locomotion", "Normal", "Outlier")) 
...
# define variables
  ...
  kernel_options <- c("linear", "polynomial", "radial", "sigmoid")
...
results_sheet <- lapply(1:nrow(options_df), function(i) {
  ...
  tryCatch({
    ...
    results[[paste0("iteration_", i)]] <- result
  }, error = function(e) {
